...
func (nm *SNotificationManager) PerformEventNotify(ctx context.Context, userCred mcclient.TokenCredential, query jsonutils.JSONObject, input api.NotificationManagerEventNotifyInput) (api.NotificationManagerEventNotifyOutput, error) {
	...
	// webconsole
	{
		var wContacts []string
		wContacts = append(wContacts, input.Contacts...)
		wContacts = append(wContacts, receiverIds...)
		wContacts = sets.NewString(receiverIds...).UnsortedList()
		if len(wContacts) > 0 {
			err := nm.create(ctx, userCred, api.WEBCONSOLE, []string{}, wContacts, input.Priority, "", message, input.Event, input.AdvanceDays)
			if err != nil {
	...
	receiverIds = make([]string, len(receivers))
	for i := range receivers {
		receiverIds[i] = receivers[i].Id
	}
	// normal contact type
	for _, ct := range contactTypes {
		err := nm.create(ctx, userCred, ct, receiverIds, input.Contacts, input.Priority, "", message, input.Event, input.AdvanceDays)
		if err != nil {
			output.FailedList = append(output.FailedList, api.FailedElem{
				ContactType: ct,
				Reason:      err.Error(),
...
=======
	p, err := n.TemplateStore().FillWithTemplate(ctx, n.Notification())
>>>>>>> feat: support event notify
	if err != nil {
		return out, err
	}
